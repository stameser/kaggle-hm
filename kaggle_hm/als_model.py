...
init_logging()
LOG = logging.getLogger('kaggle_hm.als')
...
class MatrixFactorizationPipeline:
    ...
    def fit(self, train):
        ...
        LOG.info(f'#items {len(train_items)}')
        ...
        LOG.info(f'#customers {len(train_customers)}')
...
def delta_column(to_date, t_dat: pd.Series):
    ...
    """
    delta = (to_date - t_dat).dt.days
    return 1 / (1 + np.power(delta // 7, 1))
...
def fit_model(min_items, min_customers, factors, regularization, iterations, cell_value, model_date):
    from_date = pd.to_datetime(model_date) - pd.Timedelta(days=7)
    to_date = pd.to_datetime(model_date)
    df = pd.read_parquet(data_root / 'clean' / 'transactions.parquet')
    df['delta_weeks'] = delta_column(to_date, df['t_dat'])
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_artifact(data_root / 'models' / 'pipeline.pkl')
